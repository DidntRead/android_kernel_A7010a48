	...
	const char *buf, size_t count)
{
	...
	cabc_mode = cabc;
	primary_display_setcabc(cabc);
	return count;
}

static int saturation = 0;
static ssize_t mtkfb_get_sat(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", saturation);
}

static ssize_t mtkfb_set_sat(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	int sat;
	...
	saturation = sat;
	color_set_saturation_gain(sat);
	return count;
}

static int hue = 0;
static ssize_t mtkfb_get_hue(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", hue);
}

static ssize_t mtkfb_set_hue(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	int m_hue;
	if (!(sscanf(buf, "%d", &m_hue))) 
	{
		return -EINVAL;
	}
...
static DEVICE_ATTR(cabc, S_IRUGO | S_IWUSR | S_IWGRP, mtkfb_get_cabc,
	mtkfb_set_cabc);
static DEVICE_ATTR(hue, S_IRUGO | S_IWUSR | S_IWGRP, mtkfb_get_hue,
	mtkfb_set_hue);
static DEVICE_ATTR(sat, S_IRUGO | S_IWUSR | S_IWGRP, mtkfb_get_sat,
	mtkfb_set_sat);
...
{
	return sysfs_create_group(&fbdev->fb_info->dev->kobj, &mtk_fb_attr_group);
}
