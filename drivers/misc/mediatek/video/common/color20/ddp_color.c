...
static bool custom_satgain = false;
static bool custom_hue = false;
...
void color_set_hue(int hue)
{
	...
	for (index = 0; index < 5; index++) {
		_color_reg_set(NULL, DISP_COLOR_LOCAL_HUE_CD_0 + 4 * index, u4Temp);
	}
}

void color_set_saturation_gain(int sat)
{
	if (sat < 0 || sat > 640) {
		return;
	}
	...
	_color_reg_mask(NULL, DISP_COLOR_G_PIC_ADJ_MAIN_2, sat, 0x000001FF);
}
