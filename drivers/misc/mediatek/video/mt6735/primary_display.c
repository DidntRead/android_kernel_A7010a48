...
int primary_display_user_cmd(unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == DISP_IOCTL_SET_CABC) {
		int cabc_mode_userspace = 0;
		if (copy_from_user(&cabc_mode_userspace, (void *)arg, sizeof(int))) {
			return -EFAULT;
		} else {
			primary_display_setcabc(cabc_mode_userspace);
		}
	} else if (cmd == DISP_IOCTL_AAL_GET_HIST || cmd == DISP_IOCTL_CCORR_GET_IRQ) {
#ifndef MTK_FB_CMDQ_DISABLE
		ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &handle);
		cmdqRecReset(handle);
