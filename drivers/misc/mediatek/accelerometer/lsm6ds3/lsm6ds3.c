...
struct lsm6ds3_i2c_data {
    ...
    atomic_t                selftest;
    atomic_t				filter;
    int                     cali_sw[LSM6DS3_GYRO_AXES_NUM+1];

    /*data*/
	s8                      offset[LSM6DS3_ACC_AXES_NUM+1];  /*+1: for 4-byte alignment*/
    int                     data[LSM6DS3_ACC_AXES_NUM+1];
...
static int LSM6DS3_CheckDeviceID(struct i2c_client *client)
{
	...
	if(databuf[0]!=LSM6DS3_FIXED_DEVID && databuf[0]!=LSM6DS3_FIXED_DEVID1)
	{
...
{
	struct i2c_client *client = lsm6ds3_i2c_client;
	int data[LSM6DS3_ACC_AXES_NUM] = {0};
       ...
       unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_READ_RAW_DATA:
			...
			LSM6DS3_ReadAccRawData(client, (int *)strbuf);
			if(copy_to_user(data, strbuf, strlen(strbuf)+1))
			{
				err = -EFAULT;
				break;	  
		...
		#else
			cali[LSM6DS3_AXIS_X] = (int)(sensor_data.x);
			cali[LSM6DS3_AXIS_Y] = (int)(sensor_data.y);	
			cali[LSM6DS3_AXIS_Z] = (int)(sensor_data.z);	
		#endif
		...
		#else
			sensor_data.x = (int)(cali[LSM6DS3_AXIS_X]);
			sensor_data.y = (int)(cali[LSM6DS3_AXIS_Y]);
			sensor_data.z = (int)(cali[LSM6DS3_AXIS_Z]);
		#endif
