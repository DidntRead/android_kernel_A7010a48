...
static struct regulator_consumer_supply dc1v8_consumers[] = {
	...
	REGULATOR_SUPPLY("DBVDD1", "2-001b"), 
...
#if 1 /*SPI*/
static struct regulator_consumer_supply dc1v8_consumers[] = {
	REGULATOR_SUPPLY("AVDD", "spi32766.0"), 
	REGULATOR_SUPPLY("DBVDD1", "spi32766.0"), 
	REGULATOR_SUPPLY("LDOVDD", "spi32766.0"),
	REGULATOR_SUPPLY("DBVDD2", "florida-codec"),
	REGULATOR_SUPPLY("DBVDD3", "florida-codec"),
	REGULATOR_SUPPLY("CPVDD",  "florida-codec"),
	REGULATOR_SUPPLY("SPKVDDL", "florida-codec"),
	REGULATOR_SUPPLY("SPKVDDR", "florida-codec"),
	REGULATOR_SUPPLY("CPVDD", "spi32766.0"),
};
#endif
...
static struct mfd_cell dc1v8_regulators[] = {
    [0]={
        .name = "reg-fixed-voltage",
        .platform_data = &dc1v8vdd_pdata,
        .pdata_size = sizeof(dc1v8vdd_pdata)
    },
};
			    ...
			    unsigned int mask, unsigned int target)
{
	...
	for (i = 0; i < timeout; i++) {
		...
		if (ret != 0) {
			dev_err(arizona->dev, "Failed to read reg %d: %d\n",
				reg, ret);
			continue;
		}
...
static int arizona_runtime_suspend(struct device *dev)
{
	...
	printk("Entering AoD mode complete %s\n",__func__);
	return 0;
err:
...
int arizona_dev_init(struct arizona *arizona)
{
	...
	ret = mfd_add_devices(arizona->dev, -1, dc1v8_regulators,
               	ARRAY_SIZE(dc1v8_regulators), NULL, 0,NULL);
	if (ret != 0) {
		dev_err(dev, "Failed to add mfd fix regulator: %d\n", ret);
		return ret;
	}
	...
	if (ret != 0) {
   	 ...
   	 printk("arizona %s %d  device ID: =%x \n", __func__,__LINE__,reg);         
	switch (reg) {
	case 0x5102:
	case 0x5110:
	case 0x6349:
	...
	default:
        ...
        printk("arizona in %s Unknown device ID: %x\n", __func__,reg);
		goto err_reset;
	}
    //lenovo-sw zhangrc2 add for show codec id Begin
    codec_id = 0x5110;
    //lenovo-sw zhangrc2 add for show codec id Begin
