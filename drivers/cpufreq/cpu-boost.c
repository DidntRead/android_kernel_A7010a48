...
struct cpu_sync {
    ...
    struct task_struct *issued_thread;
...
static unsigned int init_complete = 0;

static unsigned int boost_ms;
module_param(boost_ms, uint, 0660);

static unsigned int sync_threshold;
module_param(sync_threshold, uint, 0660);

static bool input_boost_enabled;
module_param(input_boost_enabled, bool, 0664);
...
void set_boost_duration(unsigned int ms) {boost_ms = ms;}
void set_freq_threshold(unsigned int freq) {sync_threshold = freq;}


EXPORT_SYMBOL(set_boost_duration);
EXPORT_SYMBOL(set_freq_threshold);
...
{
	unsigned long dest_cpu = (unsigned long) data;
	unsigned long src_cpu; int ret;
	...
	while (1) {
        ...
        pr_debug("sync wq thread starts, src:%ld, dst:%ld\n", 
            src_cpu, dest_cpu);
...
void signal_thread_to_boost(void)
{
    unsigned long cpu;
	struct cpu_sync *s;

    if (!init_complete || !boost_ms)
        return;
    if (!in_interrupt()) {
        printk(KERN_ERR "only run in ISR\n");
        return;
        }
	for_each_possible_cpu(cpu) {
		s = &per_cpu(sync_info, cpu);
...
EXPORT_SYMBOL(signal_thread_to_boost);
